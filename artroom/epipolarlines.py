...
def Compute_SSD(b_l, b_r):
    if b_l.shape != b_r.shape:
        return -1

    return np.sum((b_l - b_r)**2)
...
cam1 = np.array([[1733.74, 0, 792.27], [0, 1733.74, 541.89], [0, 0, 1]])
baseline = 536.62
...
# lines1, lines2,epipolar_img_before =getEpipolarLines(img_l, img_r, matched_final)
...
# plt.imshow(epipolar_img_before)
# plt.savefig("epip_img_before.png")
plt.imshow(epipolar_img)
plt.savefig("artroom_epipolar_img.png")
plt.imshow(disparity_map, cmap='hot', interpolation='nearest')
plt.savefig("artroom_disparity_map_heat.png")
plt.imshow(disparity_map, cmap='gray', interpolation='nearest')
plt.savefig("artroom_disparity_map_gray.png")
plt.imshow(depth_map, cmap='hot', interpolation='nearest')
plt.savefig("artroom_depth_map_heat.png")
plt.imshow(depth_map, cmap='gray', interpolation='nearest')
plt.savefig("artroom_depth_map_gray.png")
